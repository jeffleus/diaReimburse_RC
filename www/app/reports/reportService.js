...
(function() {
    ...
    angular.module('starter.services')
	 	.service('ReportSvc', ['$q', '$rootScope', '$timeout', '$cordovaFile'
                                    , 'ReportBuilderSvc', 'SettingsSvc', reportSvc]);
//
// genReportDef --> genReportDoc --> buffer[] --> Blob() --> saveFile --> return filePath
//
//  Events: ReportSvc::Progress, ReportSvc::Done
//
	 function reportSvc($q, $rootScope, $timeout, $cordovaFile, ReportBuilderSvc, SettingsSvc) {
...
//
// deleteExisting to overwrite any existing files
        function deleteExisting() {
            var filename = SettingsSvc.lastName?SettingsSvc.lastName:"reimburse"
            filename += "_" + moment().format("YYMMDD") + ".pdf";
            
            return $cordovaFile.checkFile(cordova.file.documentsDirectory, filename)
                .then(function(success) {                
                    console.log('Delete file before proceeding...');
                    return $cordovaFile.removeFile(cordova.file.documentsDirectory, filename);
                }).catch(function(error) {
                    console.log('First time file --> no need to delete...');
                });
        }
